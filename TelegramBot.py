...
async def start2(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    agent.start_chat(user.name, update.effective_user.id)
...
async def echo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    reply_text = await agent.chat(update.effective_user.id, update.message.text)
    if reply_text.strip():
        # context.user_data['chat_history'] += f'USER: {update.message.text}\nASSISTANT: {reply_text}\n'
        await update.message.reply_text(reply_text)
...
def main() -> None:
    ...
    BOT_API_TOKEN = os.getenv('BOT_API_TOKEN', None) or logger.fatal("Can't get the API Token")
    persistence = PicklePersistence(filepath="conversationbot")

    application = Application.builder().token(BOT_API_TOKEN).persistence(persistence).build()
...
agent = Agent() #TODO: move it inside
